...
class Train_SpatialMuSC:
    def __init__(self,
        ...
        self.A3_sp = self.adj['adj_spatial_omics3'].to(self.device)
        self.A1_ft = self.adj['adj_feature_omics1'].to(self.device)
        self.A2_ft = self.adj['adj_feature_omics2'].to(self.device)
        self.A3_ft = self.adj['adj_feature_omics3'].to(self.device)
        ...
        self.X3 = torch.FloatTensor(self.adata_omics3.obsm['feat'].copy()).to(self.device)

        self.in_dim_1 = self.X1.shape[1]
        self.in_dim_2 = self.X2.shape[1]
        self.in_dim_3 = self.X3.shape[1]
        self.out_dim_1 = self.dim_output
        self.out_dim_2 = self.dim_output
        self.out_dim_3 = self.dim_output


        if self.datatype == 'SPOTS':
            self.epochs = 600
            self.weight_factors = [1, 10, 10]
            self.lambda_infmask = 0.05
            lambda_lg = 0.05
        elif self.datatype == 'Stereo-CITE-seq':
            self.epochs = 800
            self.weight_factors = [1, 50, 50]
            tfm_heads = 4
        ...
        self.model = Encoder_overall(
            self.in_dim_1, self.out_dim_1, self.in_dim_2, self.out_dim_2, self.in_dim_3, self.out_dim_3,
        ...
        if self.enable_contrast:
            ...
            self.contrast3 = Contrast(hidden_dim=self.out_dim_3, tau=contrast_tau, lam=0.5).to(self.device)
        ...
        if self.enable_ddpm:
            ...
            self.model.refiner3.gen = self._ddpm_gen
    ...
    def train(self):
        ...
        if self.enable_contrast:
            ...
            self.pos_dense3 = self._build_spatial_pos_dense(self.A3_sp, N, topk=5, self_weight=0.6, neighbor_weight=0.4).to(self.device)
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            loss_recon3 = F.mse_loss(out['emb_recon_omics3'], self.X3)
            loss = self.weight_factors[0] * loss_recon1 + self.weight_factors[1] * loss_recon2 + self.weight_factors[2] * loss_recon3
            ...
            if self.enable_contrast:
                ...
                lg3 = self.contrast3(out['emb_local_omics3'], out['emb_global_omics3'], self.pos_dense3)
                contrast_loss = self.lambda_lg * (lg1 + lg2 + lg3)
            ...
            if self.enable_infmask:
                ...
                X3_m = self._feature_mask(self.X3, keep_ratio=0.2)
                out_mask = self.model(
                    X1_m, X2_m, X3_m,
                    self.A1_sp, self.A1_ft,
                    self.A2_sp, self.A2_ft,
        ...
        self.model.eval()
        with torch.no_grad():
            out = self.model(self.X1, self.X2, self.X3, self.A1_sp, self.A1_ft, self.A2_sp, self.A2_ft, self.A3_sp, self.A3_ft)

        emb1 = F.normalize(out['emb_latent_omics1'], p=2, eps=1e-12, dim=1)
        emb2 = F.normalize(out['emb_latent_omics2'], p=2, eps=1e-12, dim=1)
        emb3 = F.normalize(out['emb_latent_omics3'], p=2, eps=1e-12, dim=1)
        ...
        return {
            ...
            'emb_latent_omics3': emb3.detach().cpu().numpy(),
