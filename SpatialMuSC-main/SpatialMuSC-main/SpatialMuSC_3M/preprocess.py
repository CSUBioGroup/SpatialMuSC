...
# Portions of the code in this project are derived and adapted from SpatialGlue.
def construct_neighbor_graph(adata_omics1, adata_omics2, adata_omics3, datatype='SPOTS', n_neighbors=3):
    """
    ...
    is_same_12 = adata_omics1.uns['adj_spatial'].equals(adata_omics2.uns['adj_spatial'])
    is_same_13 = adata_omics1.uns['adj_spatial'].equals(adata_omics3.uns['adj_spatial'])
    is_same_23 = adata_omics2.uns['adj_spatial'].equals(adata_omics3.uns['adj_spatial'])
    print("same true 1-2：", is_same_12)
    print("same true 1-3：", is_same_13)
    print("same true 2-3：", is_same_23)
    ################# feature graph #################
...
def construct_graph_by_feature(adata_omics1, adata_omics2, adata_omics3, k=20, mode="connectivity", metric="correlation",
                               include_self=False):
...
def transform_adjacent_matrix(adjacent):  # transform
...
def make_plot_view_rotate(adata, mode="cw"):
    ad = adata.copy()
    XY = ad.obsm["spatial"].copy()
    if mode == "cw":        # 顺时针90°: (x',y') = ( y, -x)
        XY = np.column_stack([XY[:, 1], -XY[:, 0]])
    elif mode == "ccw":     # 逆时针90°: (x',y') = (-y,  x)  <-- 也就是 270° 顺时针
        XY = np.column_stack([-XY[:, 1], XY[:, 0]])
    else:
