...
class Encoder_overall(Module):
    ...
    Cross-modality fusion:
      [h_1 || h_2] -> MLP -> h_fused -> decoders (spatial adj) -> recon losses
    """
    def __init__(
        self,
        ...
        # ----- Cross-modality fusion -----
        self.fusion = MLP(self.latent_dim_1 + self.latent_dim_2, self.latent_dim_1, self.latent_dim_1)
    ...
    def forward(
        ...
        if self.enable_ddpm:
            ...
            global_ref_1, global_ref_2 = global_raw_1, global_raw_2
        ...
        # ===== Cross-modality fusion =====
        h_fused = self.fusion(torch.cat([h1, h2], dim=1))  # [N, d1]
